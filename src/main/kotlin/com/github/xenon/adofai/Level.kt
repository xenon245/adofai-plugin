...
class Level {
    ...
    private val courseFile: File
        get() = File(Adofai.coursesFolder, "$name.schem")
    ...
    constructor(name: String, region: CuboidRegion) {
        ...
        file = File(Adofai.levelFolder, "$name.yml")
    }
        ...
        if (file.exists()) {
            if (file.md5Digest.contentEquals(temp.md5Digest)) {
                Files.copy(
                    file.toPath(),
                    File(
                        Adofai.historyFolder.apply { mkdirs() },
    ...
    private fun Clipboard.paste() {
        ...
        val editSession = WorldEdit.getInstance().editSessionFactory.getEditSession(region.world, -1)
